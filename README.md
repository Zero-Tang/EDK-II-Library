...
## Motivation
I chose LLVM to compile NoirVisor EFI executables. However, LLVM is slow at compilation. Since there are so many source codes in EDK II, it will cost an unacceptable amount of time to compile them, even if I select only those required for NoirVisor EFI executables.
